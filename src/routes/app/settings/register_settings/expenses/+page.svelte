<script lang="ts">
	import { getExpensesTable } from './data.remote';
	...
	let tableData = $derived(expensesQuery.current?.data ?? []);
	let currentStartDate = $derived(expensesQuery.current?.currentStartDate);
	let currentEndDate = $derived(expensesQuery.current?.currentEndDate);
...
<div class="min-h-screen bg-background">
	<main class="container mx-auto space-y-6 px-4 py-6 md:px-6">
		...
		<div class="flex items-center gap-4">
			...
			<div class="space-y-1">
				<h1 class="font-mono text-3xl tracking-wider text-neutral-800 md:text-4xl">Έξοδα</h1>
				<p class="text-sm text-muted-foreground">
					Προβολή όλων των εξόδων για την επιλεγμένη περίοδο
				</p>
			</div>
		</div>
		...
		{#if currentStartDate && currentEndDate}
			<p class="text-sm text-muted-foreground">
				Showing expenses from: {formatDateRange(currentStartDate, currentEndDate)}
			</p>
		{/if}

		<!-- Data Table -->
		<div class="space-y-4">
			<DataTable data={tableData} columns={expenseColumns} bind:pagination />
		</div>
	</main>
</div>

...
export const columns: ColumnDef<Profile>[] = [
	...
	},
	{
		...
		header: 'Image',
		cell: ({ row }) => {
			const avatarSnippet = createRawSnippet<[{ imageUrl: string }]>((getImageUrl) => {
		...
		cell: ({ row }) => {
			const badgeSnippet = createRawSnippet<[{ roleName: string; badgeColor: string }]>(
				(getData) => {
					...
					return {
						render: () =>
							`<span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent text-white hover:opacity-90" style="background-color: ${badgeColor};">${roleName}</span>`
					};
				}
			);
	...
	},
	{
		...
		header: 'Created',
		cell: ({ row }) => {
			const dateSnippet = createRawSnippet<[{ createdAt: string }]>((getCreatedAt) => {

...
export const addBeverage = form(addBeverageSchema, async (beverageData) => {
	...
	try {
		...
		if (beverageData.image_url && beverageData.image_url instanceof File) {
			...
			// Get public URL
			const { data: urlData } = supabase.storage.from('beverages').getPublicUrl(uploadData.path);

			imageUrl = urlData.publicUrl;
		}

		// Insert beverage into DB
		const { error: insertError } = await supabase.from('beverages').insert({
...
export const editBeverage = form(editBeverageSchema, async (beverageData) => {
	...
	try {
		...
		const { error } = await supabase.from('beverages').update(updateData).eq('id', beverageData.id);
...
export const removeIngredientFromBeverage = command(
	removeIngredientFromBeverageSchema,
	async ({ recipeIngredientId }) => {
		const supabase = createServerClient();
...
export const uploadBeverageImage = command(
	uploadBeverageImageSchema,
	async ({ image, beverageId }) => {
		const supabase = createServerClient();

...
export const getOrgEquipment = query(getOrgEquipmentSchema, async ({ orgId }) => {
  ...
  const { data: equipment, error: equipError } = await supabase
    .from("equipment")
    .select("id, name, model, serial_number, image_url, status, last_service_date, next_service_date")
    .eq("org_id", orgId)
...
export const createEvaluation = command(createEvaluationSchema, async ({ orgId, visitDate, storeManagers, baristasOnDuty }) => {
  const supabase = createServerClient();
  const profile = await getUserProfileWithRoleCheck([3]);
...
export const updateEvaluationHeader = command(
  ...
  async ({ evaluationId, visitDate, storeManagers, baristasOnDuty, overallRating, overallComments }) => {
    const supabase = createServerClient();
    const profile = await getUserProfileWithRoleCheck([3]);
...
export const saveCheckItems = command(saveCheckItemsSchema, async ({ evaluationId, checkItems }) => {
  const supabase = createServerClient();
  const profile = await getUserProfileWithRoleCheck([3]);
...
export const saveMachineData = command(saveMachineDataSchema, async ({ evaluationId, machineData }) => {
  const supabase = createServerClient();
  const profile = await getUserProfileWithRoleCheck([3]);
...
export const saveActionItems = command(saveActionItemsSchema, async ({ evaluationId, actionItems }) => {
  const supabase = createServerClient();
  const profile = await getUserProfileWithRoleCheck([3]);
...
export const uploadEvaluationPhoto = command(
  uploadPhotoSchema,
  async ({ evaluationId, sectionKey, photoLabel, caption, displayOrder, fileName, fileBase64, fileType }) => {
    const supabase = createServerClient();
    const profile = await getUserProfileWithRoleCheck([3]);

<script lang="ts">
  ...
  const toolbarButtons = [
    ...
    { action: applyStrikethrough, icon: Strikethrough, label: 'Strikethrough', shortcut: 'Ctrl+Shift+S' },
    { separator: true },
    ...
    { action: applyNumberedList, icon: ListOrdered, label: 'Numbered List', shortcut: 'Ctrl+Shift+O' },
...
>
  ...
  <div class="flex items-center justify-between border-b border-border bg-muted/50 px-3 py-2">
    ...
    <ToggleGroup type="single" value={mode} onValueChange={(v) => v && (mode = v as 'edit' | 'preview')}>
  ...
  >
    {#if mode === 'edit'}
      <textarea
        ...
        class={cn(
          'w-full h-full resize-none border-0 bg-background p-4 text-foreground',
...
<Dialog.Root bind:open={imageDialogOpen}>
  <Dialog.Content class="sm:max-w-md">
    ...
    <Dialog.Footer>
      <Button variant="outline" onclick={() => (imageDialogOpen = false)}>
        Cancel
      </Button>

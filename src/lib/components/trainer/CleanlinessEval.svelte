<script lang="ts">
	...
	import { Plus, X, ChevronDown, ChevronUp, Sparkles } from 'lucide-svelte';
	...
	// Pull cleanliness items from the grouped derived
	const section = $derived(store.groupedSections.find((g) => g.section === 'cleanliness')!);

	function handleAddItem() {
		if (!newItemLabel.trim()) return;
		store.addItem('cleanliness', newItemLabel.trim());
		newItemLabel = '';
		addingNew = false;
	}
</script>
...
<div class="flex flex-col gap-4 py-4">
	...
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-2">
			...
			<div>
				<h4 class="text-lg font-semibold tracking-tight">Καθαριότητα</h4>
			...
			{#each section.items as item (item.id)}
				...
				>
					...
					<div class="flex min-w-0 flex-1 flex-col gap-1.5">
						...
						<!-- Score slider — only visible when checked -->
						{#if item.checked}
							<div class="flex items-center gap-2">
								<Slider
									class="flex-1"

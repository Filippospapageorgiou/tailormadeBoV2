<script lang="ts">
    ...
    let selectManagers = $derived(employees.filter(n => n.is_manager) ?? []);
        ...
        <Popover.Root bind:open={managersOpen}>
            <Popover.Trigger>
                <Button
                    variant="outline"
                    role="combobox"
                    class="w-full justify-between text-muted-foreground font-normal"
                >
                    Επιλογή υπεύθυνου...
                        ...
                        {#each selectManagers as emp}
                            ...
                            >
                                <div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold text-white"
                                    >
                                    {#if emp.image_url}
                                        <img src={emp.image_url} alt={emp.full_name} class="h-6 w-6 rounded-full object-cover dark:bg-white" />
                                    {:else}
        ...
        {#if managers.length > 0}
            <div class="flex flex-wrap gap-2">
                {#each managers as id}
                    ...
                    {#if emp}
                        <Badge variant="secondary" class="flex items-center gap-1.5 pl-1 pr-1 py-1">
                            <div class="flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[9px] font-bold text-white overflow-hidden"
                                >
                                {#if emp.image_url}
                                    <img src={emp.image_url} alt={emp.full_name} class="h-5 w-5 object-cover dark:bg-white" />
                                {:else}
                                    {getInitials(emp.full_name)}
                                {/if}
                            </div>
                            ...
                            <button
                                ...
                                class="ml-0.5 rounded-full hover:bg-muted p-0.5 transition-colors"
                            >
                                <X class="h-3 w-3" />
                            </button>
                        </Badge>
                    ...
                    <Command.Group>
                        {#each selectBaristas as emp}
                            <Command.Item
                                ...
                                class="flex items-center gap-2 cursor-pointer"
                            >
                                <div class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold text-white overflow-hidden">
                                    {#if emp.image_url}
                                        <img src={emp.image_url} alt={emp.full_name} class="h-6 w-6 rounded-full object-cover dark:bg-white" />
                                    {:else}
                                        {getInitials(emp.full_name)}

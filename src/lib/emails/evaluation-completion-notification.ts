// src/lib/emails/evaluation-completion-notification.ts
...
function generateCompletionEmailTemplate(params: EvaluationCompletionParams): string {
	const { recipientName, trainerName, storeName, storeLocation, visitDate, evaluationId } = params;

	const evalUrl = `${PUBLIC_APP_URL}/app/trainers/evaluations/${evaluationId}`;
	...
	return `
		...
		<head>
			...
			<title>Ολοκλήρωση Αξιολόγησης</title>
		</head>
							...
							<tr>
								<td style="padding: 0 48px 48px;">
									...
									<div style="text-align: center; margin-bottom: 28px;">
										<span style="display: inline-block; padding: 6px 20px; background-color: #F0F9F4; border: 1px solid #C6E8D4; border-radius: 20px; color: #2D7A4F; font-size: 13px; font-family: 'Segoe UI', system-ui, sans-serif; letter-spacing: 0.3px;">
									...
									<!-- Visit date -->
									<div style="margin: 0 0 32px; padding: 16px; background-color: #F9F6F2; border-radius: 4px; text-align: center;">
							...
							<tr>
								<td style="padding: 24px 48px; background-color: #F9F6F2;">
									<p style="margin: 0 0 8px; color: #B5A898; font-size: 11px; text-align: center;">
										Υποβλήθηκε: ${new Date().toLocaleDateString('el-GR', { day: 'numeric', month: 'long', year: 'numeric' })}
									</p>
									<p style="margin: 0; color: #C8BDB0; font-size: 10px; text-align: center;">
										Αν έχετε ερωτήσεις, επικοινωνήστε με τον trainer.
									</p>
								</td>
							</tr>
						</table>
...
export async function sendEvaluationCompletionNotification(
	params: EvaluationCompletionParams
): Promise<{ success: boolean; message: string; emailId?: string }> {
	try {
		const { recipientEmail, trainerName, storeName } = params;

		const emailHtml = generateCompletionEmailTemplate(params);

		const { data, error } = await resend.emails.send({
			from: 'TailorMade <team@tailormadebov2.app>',
			to: recipientEmail,
			subject: `✓ Ολοκλήρωση αξιολόγησης: ${storeName} — ${trainerName}`,
			html: emailHtml
		});

		if (error) {
			console.error('[sendEvaluationCompletionNotification] Resend error:', error);
			return { success: false, message: `Failed to send: ${error.message}` };
		}

		console.log('[sendEvaluationCompletionNotification] Email sent to:', recipientEmail);
		return { success: true, message: `Email sent to ${recipientEmail}`, emailId: data?.id };
	} catch (err) {
		console.error('[sendEvaluationCompletionNotification] Unexpected error:', err);
